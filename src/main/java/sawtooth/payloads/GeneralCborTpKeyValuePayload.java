...
public class GeneralCborTpKeyValuePayload implements ISawtoothWritePayload {

    private static final Logger LOG = Logger.getLogger(GeneralCborTpKeyValuePayload.class);
    ...
    public List<String> getInputAddresses(final String... params) {
        if ("Get".equals(function)) {
            return TpKeyValueHandler.getTpInputWrite(Configuration.TP_ENUM, Configuration.TP_PREFIX,
    ...
    public List<String> getOutputAddresses(final String... params) {
        if("Set".equals(function)) {
            return TpKeyValueHandler.getTpOutputWrite(Configuration.TP_ENUM, Configuration.TP_PREFIX,
                    Arrays.asList(params));
        } else {
            return Collections.EMPTY_LIST;
        }
