package graphene.payload_patterns.loop;
...
public class GrapheneUniformLoopPayload implements IGraphenePayloads {

    private static final Logger LOG = Logger.getLogger(GrapheneUniformLoopPayload.class);
    ...
    private ImmutablePair<Object[], String> createPayload(final ClientObject clientObject, final String acctId,
        ...
        int operationId = OperationType.LOOP_OPERATION.ordinal();

        iGrapheneWritePayload.setBaseOperation(operationId);

        List<String> params = new ArrayList<>();
        params.add(String.valueOf(Configuration.START_LOOP));
        params.add(String.valueOf(Configuration.END_LOOP));
        ...
        iGrapheneWritePayload.setEventPrefix("loop ");
        iGrapheneWritePayload.setSpecificPayloadType("loop");
