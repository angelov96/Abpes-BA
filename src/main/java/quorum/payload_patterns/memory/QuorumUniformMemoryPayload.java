package quorum.payload_patterns.memory;
...
import quorum.payload_patterns.loop.QuorumUniformLoopPayload;
...
public class QuorumUniformMemoryPayload implements IQuorumPayloads {

    private static final Logger LOG = Logger.getLogger(QuorumUniformLoopPayload.class);
    ...
    private ImmutablePair<Object[], String> createPayload(final ClientObject clientObject, final int i,
        ...
        String function = "createString";
        List<Type<?>> inputs = new ArrayList<>();
        inputs.add(new Uint(BigInteger.valueOf(Configuration.LEN_OUTER_LOOP_MEMORY)));
        inputs.add(new Uint(BigInteger.valueOf(Configuration.LEN_INNER_LOOP_MEMORY)));
        inputs.add(new Uint(BigInteger.valueOf(Configuration.FIRST_CHAR_INT_MEMORY)));
        inputs.add(new Uint(BigInteger.valueOf(Configuration.LENGTH_MEMORY)));
        ...
        iQuorumWritePayload.setEventPrefix("memory");
        iQuorumWritePayload.setSpecificPayloadType("memory");
