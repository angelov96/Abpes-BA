package quorum.payload_patterns.loop;
...
public class QuorumUniformLoopPayload implements IQuorumPayloads {
    ...
    private ImmutablePair<Object[], String> createPayload(final ClientObject clientObject, final int i,
        ...
        String function = "loopFunc";
        List<Type<?>> inputs = new ArrayList<>();
        inputs.add(new Uint(BigInteger.valueOf(Configuration.START_LOOP)));
        inputs.add(new Uint(BigInteger.valueOf(Configuration.END_LOOP)));
        ...
        iQuorumWritePayload.setEventPrefix("loop");
        iQuorumWritePayload.setSpecificPayloadType("loop");
